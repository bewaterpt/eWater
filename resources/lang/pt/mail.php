<?php
    return [
        'go_to_model' => 'Ver Interrupção',
        'fallback_link' => 'Se o botão acima não está a funcionar, clique aqui: <a href=":url">:url</a>',
        'interruptions' => [
            'scheduled' => [
                'created_subject' => "Nova Interrupção Programada - Be Water Ourém",
                'created' => "Foi criada uma interrupção Programada com os seguintes dados: ",
                'updated_subject' => "Interrupção Programada Atualizada - Be Water Ourém",
                'updated' => "Foi atualizada uma interrupção Programada, os novos dados são os seguintes:",
                'canceled_subject' => "Interrupção Programada Cancelada - Be Water Ourém",
                'canceled' => "Foi cancelada uma interrupção Programada com os seguintes dados:",
            ],
            'unscheduled' => [
                'created_subject' => "Nova Interrupção Não Programada - Be Water Ourém",
                'created' => "Foi criada uma interrupção Não Programada com os seguintes dados: ",
                'updated_subject' => "Interrupção Programada Não Atualizada - Be Water Ourém",
                'updated' => "Foi atualizada uma interrupção Não Programada, os novos dados são os seguintes:",
                'canceled_subject' => "Interrupção Não Programada Cancelada - Be Water Ourém",
                'canceled' => "Foi cancelada uma interrupção Não Programada com os seguintes dados:",
            ],
            'ref' => 'Ref.ª Interna',
            'disclaimer' => "Os conteúdos desta mensagem são de carácter interno e confidencial, apenas dados relativos ao procedimento são apresentados para os respectivos responsáveis do mesmo, de forma meramente informativa e independente do fluxo do processo em causa. No caso de existir alguma informação errada queiram, por favor, informar os serviços de atendimento da <b>:company</b> através do número de apoio <b>:phone</b> ou pelo e-mail <b><a href='mailto://:email'>:email</a></b>. Poderão consultar as interrupções atualmente publicadas através da seguinte <a href=':website/interrupcoes'>ligação</a><br><br><u>Esta mensagem foi gerada automaticamente, não deve responder à mesma pois não será processada.</u>"
        ],
    ];
?>
